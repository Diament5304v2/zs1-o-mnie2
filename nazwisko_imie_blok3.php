...
<table>
    ...
    <tbody>
        <?php foreach ($zawodnicy as $zawodnik): ?>
            ...
            $sredniCzas = round($suma / count($zawodnik['okrazenia']));
    ...
    <tfoot>
        ...
        $suma = 0;
        foreach ($wszystkieOkrazenia as $okrazenie) {
            $suma += czasZKarami($okrazenie);
        }
        $sredniaZawodow = round($suma / count($wszystkieOkrazenia));

        $najlepszeZawodow = null;
        foreach ($wszystkieOkrazenia as $okrazenie) {
            $czas = czasZKarami($okrazenie);
            if ($najlepszeZawodow === null || $czas < $najlepszeZawodow) {
                $najlepszeZawodow = $czas;
            }
        }
        ?>
        <tr>
